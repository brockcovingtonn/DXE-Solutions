...
export default function BidViewActions({ bidId, projectId, hasPdf }) {
  ...
  async function duplicate() {
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/bids/${bidId}/duplicate`, { method: 'POST' });
  ...
  return (
    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
      {hasPdf && (
        <a href={`/api/admin/bids/${bidId}/download`} className="btn-navy">
