...
38. Run `supabase/employee_accounting_templates_migration.sql` any time after `client_employee_contacts_migration.sql`, `templates_migration.sql`, and `invoices_migration.sql`. Adds `shared_with_employees` to `document_templates` (admin chooses which templates employees can see, powering the new employee **Templates** tab) and `approval_status`/`visible_to_client` to `invoices` (an employee can submit a reimbursement or a project-related receipt from the new employee **Accounting** tab; it lands `pending` and invisible to the client until an admin approves it and explicitly shares it). **Behavior change**: clients previously saw every invoice/receipt on their project unconditionally — `visible_to_client` now defaults to `false` for every entry, including admin-created ones, so sharing with the client is always an explicit action from here on. The employee **Contacts** tab needed no new RLS — `client_employee_contacts_migration.sql` already scoped it.
