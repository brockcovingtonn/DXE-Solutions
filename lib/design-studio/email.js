  ...
  const html = `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto; color: ${NAVY};">
      ...
      <div style="padding: 28px 24px; border: 1px solid #DCE5EC; border-top: none; background: #FFFFFF;">
        ...
        <p>
          ${isEsign
            ? `Your proposal (${escapeHtml(quote.quote_number)}) is ready for your signature${quote.project_address ? ` for <strong>${escapeHtml(quote.project_address)}</strong>` : ''}. Please review the details below and sign electronically to approve.`
            : `Thank you for the opportunity to work with you${quote.project_address ? ` on <strong>${escapeHtml(quote.project_address)}</strong>` : ''}. Your proposal (${escapeHtml(quote.quote_number)}) is ready to view.`}
        </p>
        ...
        <p style="margin-top: 18px;">
          ${isEsign
            ...
            : "Please review the proposal at the link below. If everything looks good, reply to this email and we'll get started — or let us know if you have any questions."}
        </p>
      ...
      <div style="background: ${CREAM}; padding: 14px 24px; text-align: center;">
        <p style="font-size: 0.7rem; color: #8296a6; letter-spacing: 0.04em; margin: 0;">
          DXE Solutions | Higher Thinking Consulting · dxesolutions.com
        </p>
      </div>
    </div>
  `;
...
export async function notifyStaffOfProposalDecision({ quote, decision, declineReason }) {
  ...
  const resend = getResend();
  if (resend && process.env.ESTIMATE_NOTIFICATION_EMAIL) {
    const html = `
