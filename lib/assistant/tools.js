...
async function listTrainingSteps({ supabase }, input) {
  let query = supabase.from('training_steps').select('project_type, title, description, sort_order').order('project_type').order('sort_order');
  if (input.category) query = query.eq('project_type', input.category);
