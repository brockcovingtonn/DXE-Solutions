...
// Kept in sync with the palette used to backfill projects.color in
// supabase/project_color_migration.sql.
export const PROJECT_COLOR_PALETTE = [
  '#3E5468', '#C9A857', '#8E6C88', '#5B8266',
...
// Toggleable scope catalog for the proposal builder (Admin → project →
// Create A Proposal). Clicking one on adds a pre-filled line item to the
// proposal (category, default description, default unit); the admin can
// still edit or remove any of it, or add a fully custom line item.
// Grouped so the picker can show them under headings.
export const PROPOSAL_SCOPE_GROUPS = [
  {
...
export const PROPOSAL_SCOPE_ITEMS = Object.fromEntries(
  PROPOSAL_SCOPE_GROUPS.flatMap((g) => g.items).map((item) => [item.key, item])
);

export const PROPOSAL_UNITS = ['LS', 'SF', 'SQ', 'LF', 'EA', 'HR', 'DAY'];
