...
// calendar.freebusy is a separate, narrow scope from calendar.events —
// Google's freeBusy.query endpoint 403s without it even though it's the
// same API. Added for the public booking page's conflict check; existing
// event-sync keeps working under calendar.events regardless. Anyone
// already connected before this change needs to reconnect once to grant
// the new scope — Google doesn't retroactively add it to an existing
// grant.
...
// Busy intervals (`[{ start, end }]`, ISO strings) for this user's
// connected calendar within [timeMinISO, timeMaxISO) — used by the public
// booking page to compute real open slots. Returns `null` (not `[]`) if
// the query itself failed (expired grant missing the calendar.freebusy
// scope, network error, etc.) — deliberately distinct from "no busy
// events found" so callers never mistake "couldn't check" for "confirmed
// clear" and offer a slot that might actually double-book.
export async function getFreeBusy(adminClient, userId, timeMinISO, timeMaxISO) {
  const conn = await getValidAccessToken(adminClient, userId);
  if (!conn) return null;
  ...
  const res = await fetch(`${GOOGLE_CALENDAR_API}/freeBusy`, {
    ...
    body: JSON.stringify({ timeMin: timeMinISO, timeMax: timeMaxISO, items: [{ id: conn.calendarId }] }),
  });
  ...
  const data = await res.json();
  const calendar = data.calendars?.[conn.calendarId];
