...
function wrapEmail({ heading, bodyHtml, ctaLabel, ctaUrl }) {
  const logoUrl = `${SITE_URL}/images/logo-slate.png`;

  return `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 480px; margin: 0 auto; color: #3E5468;">
      <div style="background: #E5ECF1; padding: 24px; text-align: center; border-bottom: 1px solid #DCE5EC;">
        <img src="${logoUrl}" alt="DXE Solutions" height="32" style="height: 32px; width: auto; display: inline-block;" />
      </div>
      <div style="padding: 24px; border: 1px solid #DCE5EC; background: #FFFFFF;">
        <h2 style="font-weight: 500; margin-top: 0;">${escapeHtml(heading)}</h2>
        ${bodyHtml}
        ${
          ctaUrl
            ? `<p style="margin-top: 24px;">
                <a href="${ctaUrl}" style="display: inline-block; background: #3E5468; color: #FFFFFF; padding: 12px 24px; text-decoration: none; font-weight: 600; letter-spacing: 0.03em;">${escapeHtml(ctaLabel || 'View in portal')}</a>
              </p>`
            : ''
        }
      </div>
      <p style="font-size: 0.75rem; color: #93A6B7; text-align: center; margin-top: 16px;">
