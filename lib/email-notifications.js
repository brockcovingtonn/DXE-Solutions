...
/**
 * Emails a finalized bid PDF to the client (or whoever the admin
 * addressed it to). The PDF is attached directly rather than linked —
 * bids aren't exposed anywhere in the client portal.
 */
export async function sendBidEmail({ toEmail, clientName, bidTitle, projectName, total, pdfBytes, fileName }) {
  const resend = getResend();
  ...
  const html = wrapEmail({
    heading: bidTitle || 'Your Bid',
    bodyHtml: `
      <p>Hi ${escapeHtml((clientName || '').split(' ')[0] || 'there')},</p>
      <p>Please find attached the bid for <strong>${escapeHtml(projectName || 'your project')}</strong>.</p>
      ${field('Total Bid Price', escapeHtml(totalStr))}
      <p style="margin-top: 18px; color: #718096; font-size: 0.85rem;">
        Reply to this email with any questions — we're happy to walk through anything on the bid.
      </p>
    `,
  });
  ...
  try {
    const result = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL,
      to: toEmail,
      subject: `${bidTitle || 'Bid'} — ${projectName || 'DXE Solutions'}`,
      html,
      attachments: [
        {
          filename: fileName || 'bid.pdf',
          content: Buffer.from(pdfBytes).toString('base64'),
        },
      ],
    });
    if (result?.error) console.error('sendBidEmail Resend error:', result.error);
    return result;
  } catch (err) {
    console.error('sendBidEmail error:', err);
    return { error: true };
  }
}
