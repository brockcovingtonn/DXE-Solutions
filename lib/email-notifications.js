...
import { DESIGN_STUDIO_CONTACT } from './design-studio/brand';
...
// Resend's SDK often "succeeds" (no thrown exception) while still
// reporting a delivery failure via result.error — most of the notify
// functions below never checked that, so a bad address or an outage
// failed completely silently. Callers that need to know whether an
// email actually went out (payment reminders, currently — see
// app/api/cron/send-payment-reminders) use this instead of calling
// resend.emails.send() directly.
async function sendAndReport(payload) {
  const resend = getResend();
  if (!resend) return { sent: false, error: 'Email is not configured.' };
  try {
    const result = await resend.emails.send({ from: process.env.RESEND_FROM_EMAIL, ...payload });
    if (result?.error) {
      return { sent: false, error: result.error.message || 'Delivery failed.' };
    }
    return { sent: true };
  } catch (err) {
...
/**
 * Confirms receipt to whoever submitted the "Book a 15-min call" form.
 * Only sent when they gave an email (there's no SMS channel). When
 * `intakeUrl` is provided — a Design Studio submission, which creates a
 * lead behind the scenes (see lib/design-studio/leads.js) — the CTA links
 * there instead of a generic "we'll be in touch".
 */
export async function sendEstimateConfirmationEmail({ firstName, email, intakeUrl }) {
  if (!email) return { sent: false, error: 'No email on file.' };

  const html = wrapEmail({
    heading: 'Thanks for reaching out',
    bodyHtml: `
      <p>Hi ${escapeHtml(firstName || 'there')},</p>
      <p>We've received your request and will be in touch within one business day to schedule your call.</p>
      ${intakeUrl ? '<p>In the meantime, a few quick details about your project would help us prepare:</p>' : ''}
    `,
    ctaLabel: intakeUrl ? 'Tell us more' : undefined,
    ctaUrl: intakeUrl,
  });

  return sendAndReport({
    to: email,
    // Only Design Studio submissions carry an intakeUrl — that's what makes
    // this a "Design Studio email" the studio's own inbox should also see.
    ...(intakeUrl ? { bcc: DESIGN_STUDIO_CONTACT.email } : {}),
    subject: 'We received your request — DXE Solutions',
    html,
  });
}

/**
 * Confirms a specific booked time once a visitor picks a slot on the
 * public booking page (see app/api/book-call/route.js). Separate from
 * sendEstimateConfirmationEmail, which just confirms the form was
 * received — this one only fires after a real calendar slot is reserved.
 */
export async function sendBookingConfirmationEmail({ firstName, email, startTime, timezone }) {
  if (!email) return { sent: false, error: 'No email on file.' };
  ...
  const html = wrapEmail({
    ...
    bodyHtml: `
      <p>Hi ${escapeHtml(firstName || 'there')},</p>
      <p>Your call is confirmed for:</p>
      ${field('When', escapeHtml(formatted))}
      <p>We'll call the number or email you provided. Need to change it? Just reply to this email.</p>
    `,
  });

  return sendAndReport({
    to: email,
    subject: 'Your call is booked — DXE Solutions',
    html,
  });
}
...
export async function notifyAdminOfUpcomingPayment({ projectName, projectId, description, amount, dueDate, daysOut }) {
  if (!process.env.ESTIMATE_NOTIFICATION_EMAIL) return { sent: false, error: 'No admin notification address configured.' };
...
export async function sendScheduledInvoiceEmail({ clientEmail, clientNotificationsEnabled, clientName, projectName, projectId, description, amount }) {
  if (clientNotificationsEnabled === false) return { sent: false, error: null }; // opted out, not a failure
  if (!clientEmail) return { sent: false, error: 'Client has no email on file.' };
